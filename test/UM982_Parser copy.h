...
private:
  ...
  */
  ...
  public:
    UM982ParserStringify(UM982Parser<S> *inParent, uint8_t inPos) :
      mPos(inPos),
      mParent(inParent)
    {
  ...
  */
  ...
  {
    //Serial.println(mCRCBuffer);
    //Serial.println(mCRC32);
 ...
 */
  ...
  void addHandler(const char *inToken, UM982Handler inHandler)
  {
    ...
    //Serial.println(*mHandlers[mHandlerCount - 1].mToken);
  }
  ...
  void operator<<(char inChar)
  {
    ...
    switch (mState) {
      ...
      case SENT:
        ...
        if ( (isalnum(inChar)) ) {
        //if ( (isUnichars(inchar))) {
          if (spaceAvail()) {
            // Increase mIndex from 5 to 13 to accomdate UM982 extended UM982 types and Uincore messages.
            if (mIndex < 14) {
